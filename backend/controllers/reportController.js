...
const getCategorySales = (req, res) => {
    const sql = "SELECT c.id, c.name AS category_name, COUNT(oi.id) AS total_orders, SUM(oi.subtotal) AS total_revenue, AVG(oi.subtotal) AS avg_item_value FROM order_items oi INNER JOIN products p ON p.id = oi.product_id RIGHT JOIN categories c ON c.id = p.category_id GROUP BY c.id, c.name HAVING SUM(oi.subtotal) > 0 ORDER BY total_revenue DESC";
    db.query(sql, (err, results) => {
        if (err) return res.status(500).json({message: "Server error."});
        res.json(results);
    });
